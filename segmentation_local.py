...
def train(model, device, train_loader, criterion, optimizer):
    ...
    for batch_idx, batch_data in enumerate(train_loader):
        inputs, targets = batch_data['input'].to(
            device), batch_data['target'].to(device)
        ...
        train_iou += sum(batch_iou)/len(batch_iou)

        # Compute batch loss and add to total
        train_loss += loss.item()
        
    # Calculate the average loss and mIoU over the entire dataset
...
def val(model, device, val_loader, criterion):
    ...
    with torch.no_grad():
        for batch_idx, sample_batched in enumerate(val_loader):
            ...
            val_loss += loss.item()
            # assuming only one batch is passed at a time
            val_iou += iou(output, target)[0]
...
def main():
    ...
    epoch, max_epochs = 1, 1  # TODO: you may want to make changes here
    best_miou = float('-inf')
