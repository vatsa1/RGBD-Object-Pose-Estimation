...
def train(model, device, train_loader, criterion, optimizer):
    ...
    for batch_idx, batch_data in enumerate(train_loader):
        ...
        targets = batch_data['target'].to(device, dtype=torch.long)
        ...
        train_iou += sum(batch_iou)

        # Compute batch loss and add to total
        train_loss += loss.item() * inputs.size(0)
        n += inputs.size(0)

    return train_loss/n, train_iou/n
...
def val(model, device, val_loader, criterion):
    ...
    with torch.no_grad():
        for batch_idx, sample_batched in enumerate(val_loader):
            data = sample_batched['input'].to(device)
            target = sample_batched['target'].to(device, dtype=torch.long)
            output = model(data)
            loss = criterion(output, target)

            val_loss += loss.item() * data.size(0)
            val_iou += sum(iou(output, target))
            n += data.size(0)
...
def main():
    ...
    train_dataset = RGBDataset(train_dir, has_gt=True, is_train=True)
    ...
    max_epochs = 50  # TODO: you may want to make changes here
